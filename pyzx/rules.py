...
def pivot(g: BaseGraph[VT,ET], matches: List[MatchPivotType[VT]]) -> RewriteOutputType[ET,VT]:
    ...
    for m in matches:
        ...
        if len(g.get_params(m[0])) == 0 and len(g.get_params(m[1])) == 0:
            if g.phase(m[0]) and g.phase(m[1]): # no params and both non-zero
                g.scalar.add_phase(Fraction(1))
        elif (g.phase(m[0]) == 0 or g.phase(m[0]) == 1) and (g.phase(m[1]) == 0 or g.phase(m[1]) == 1): # one or more has params, and both have 0 or 1 const
            psA = g.get_params(m[0])
            psB = g.get_params(m[1])
            if (g.phase(m[0]) == 1): psA = psA.symmetric_difference({"1"})
            if (g.phase(m[1]) == 1): psB = psB.symmetric_difference({"1"})
            g.scalar.add_phase_vars_pi_pair(psA,psB)
        elif (g.phase(m[0]) == 0 or g.phase(m[0]) == 1) or (g.phase(m[1]) == 0 or g.phase(m[1]) == 1): # only one has a 0 or 1 const
            if (g.phase(m[0] == 0)): g.scalar.add_phase_vars_pi(g.get_params(m[0]))
            elif (g.phase(m[0] == 1)): g.scalar.add_phase_vars_pi(g.get_params(m[0]).symmetric_difference({"1"}))
            if (g.phase(m[1] == 0)): g.scalar.add_phase_vars_pi(g.get_params(m[1]))
            elif (g.phase(m[1] == 1)): g.scalar.add_phase_vars_pi(g.get_params(m[1]).symmetric_difference({"1"}))
...
def match_ids_parallel(
    ...
    while (num == -1 or i < num) and len(candidates) > 0:
        v = candidates.pop()
        if phases[v] != 0 or not vertex_is_zx(types[v]) or g.is_ground(v) or len(g.get_params(v)) > 0:
            continue
        neigh = g.neighbors(v)
...
def match_phase_gadgets(g: BaseGraph[VT,ET]) -> List[MatchGadgetType[VT]]:
    ...
    for v in g.vertices():
        if phases[v] != 0 and hasattr(phases[v], 'denominator') and phases[v].denominator > 2 and len(list(g.neighbors(v)))==1:
            ...
            if len(g.get_params(v)) > 0: continue #TEMP?
            gadgets[n] = v
...
def apply_supplementarity(
    ...
    for v, w, t, neigh in matches:
        ...
        alpha = g.phase(v)
        beta = g.phase(w)
        #alpha_params = g.get_params(v)
        #beta_params = g.get_params(w)
        g.scalar.add_power(-2*len(neigh))
        #print("\ntype",t," with phase=",alpha," and params=",alpha_params) #TEMP
